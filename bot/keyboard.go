...
func CreateInlineKeyBoard(callback map[string]string, column int) tgbotapi.InlineKeyboardMarkup {
	...
	//TODO проверять кратность строк и колонок. Если возможна пустая кнопка - подбирать допустимый размер. В дальнейшем убрать колонку, и определять автоматически
